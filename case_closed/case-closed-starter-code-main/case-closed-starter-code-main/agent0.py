...
def send_move():
    ...
    agent = GLOBAL_GAME.agent1 if player_number == 1 else GLOBAL_GAME.agent2
    other_agent = GLOBAL_GAME.agent2 if player_number == 1 else GLOBAL_GAME.agent1
    
    head = agent.trail[-1]
    direction = agent.direction
    dx, dy = direction.value
    next_head = (head[0] + 2 * dx, head[1] + 2 * dy)
    
    next_head = GLOBAL_GAME.board._torus_check(next_head)
    
    # move = ""
    # if other_agent and other_agent.alive and next_head in other_agent.trail:
